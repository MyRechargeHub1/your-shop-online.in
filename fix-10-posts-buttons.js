...
console.log('Adding top button to 10 posts...\n');
...
for (const file of posts) {
    ...
    // Add button after featured image, before post-content
    const pattern = /(<\/div>)\s*(<div class="post-content">)/;
    content = content.replace(pattern, `$1${buyButton}\n\n            $2`);

    fs.writeFileSync(filepath, content, 'utf8');
    const count = (content.match(/🛒 BUY NOW/g) || []).length;
    console.log(`✅ ${file}: Now has ${count} buttons`);
}
